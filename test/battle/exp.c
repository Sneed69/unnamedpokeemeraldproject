...
WILD_BATTLE_TEST("Experience is gained by mons which did not participate in battle")
{
...
WILD_BATTLE_TEST("Experience is not gained by eggs")
{

    GIVEN {
        PLAYER(SPECIES_ALAKAZAM);
        PLAYER(SPECIES_ABRA) { Level(1); Egg(TRUE); }
        OPPONENT(SPECIES_CATERPIE) { Level(10); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE); }
    } SCENE {
        ...
        EXPECT_EQ(GetMonData(&gPlayerParty[1], MON_DATA_EXP), gExperienceTables[gSpeciesInfo[SPECIES_ABRA].growthRate][1]);
    }
}

WILD_BATTLE_TEST("Experience is gained by fainted mons")
{
