...
AI_SINGLE_BATTLE_TEST("AI sees increased base power of Flail")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_ALAKAZAM) { HP(hp); MaxHP(490); Speed(20); Moves(MOVE_BODY_SLAM, MOVE_FLAIL); }
    } WHEN {
        TURN { EXPECT_MOVE(opponent, expectedMove); }
    }
}
...
AI_SINGLE_BATTLE_TEST("AI will only use Dream Eater if target is asleep")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_ALAKAZAM) { Moves(MOVE_BODY_SLAM, MOVE_DREAM_EATER); }
    } WHEN {
        TURN { EXPECT_MOVE(opponent, expectedMove); }
    } SCENE {
...
AI_SINGLE_BATTLE_TEST("AI sees increased base power of Spit Up")
{
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_ALAKAZAM) { HP(43); }
        OPPONENT(SPECIES_ALAKAZAM) { Moves(MOVE_STOCKPILE, MOVE_SPIT_UP, MOVE_TACKLE); }
    } WHEN {
