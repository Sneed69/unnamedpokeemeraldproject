...
SINGLE_BATTLE_TEST("Pursuit becomes a locked move after being used on switch-out while holding a Choice Item")
{
    GIVEN {
        ...
        PLAYER(SPECIES_ALAKAZAM) { Item(ITEM_CHOICE_BAND); MovesWithPP({MOVE_PURSUIT, 1}, {MOVE_CELEBRATE, 10}, {MOVE_WATER_GUN, 10}, {MOVE_TACKLE, 10}); }
        OPPONENT(SPECIES_ABRA);
        OPPONENT(SPECIES_ALAKAZAM);
    } WHEN {
        ...
        MESSAGE("Alakazam used Struggle!");
