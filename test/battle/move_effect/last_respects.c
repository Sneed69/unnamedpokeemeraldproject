...
SINGLE_BATTLE_TEST("Last Respects power is multiplied by the amount of fainted mon in the user's side - Player", s16 damage)
{
    ...
    GIVEN {
        PLAYER(SPECIES_GOLEM); // Not Alakazam to omit type effectiveness
        PLAYER(SPECIES_GEODUDE);
        OPPONENT(SPECIES_ALAKAZAM) { HP(500); Defense(121);  Item(ITEM_LEPPA_BERRY); Moves(MOVE_RECYCLE, MOVE_NONE, MOVE_NONE, MOVE_NONE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Last Respects power is multiplied by the amount of fainted mon in the user's side - Opponent", s16 damage)
{
    ...
    PARAMETRIZE { faintCount = 2; }
    GIVEN {
        PLAYER(SPECIES_ALAKAZAM) { HP(500); Defense(121); Item(ITEM_LEPPA_BERRY); Moves(MOVE_RECYCLE, MOVE_NONE, MOVE_NONE, MOVE_NONE); }
        OPPONENT(SPECIES_GOLEM); // Not Alakazam to omit type effectiveness
