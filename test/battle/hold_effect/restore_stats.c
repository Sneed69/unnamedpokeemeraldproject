...
SINGLE_BATTLE_TEST("White Herb restores stats when they're lowered")
{
    GIVEN {
        ...
        PLAYER(SPECIES_ALAKAZAM) { Item(ITEM_WHITE_HERB); }
...
{
    GIVEN {
        OPPONENT(SPECIES_ALAKAZAM) { Item(ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_ABRA) { Item(ITEM_WHITE_HERB); }
        ...
        MESSAGE("Foe Alakazam's White Herb restored its status!");

        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, opponentRight);
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, opponentRight);
        MESSAGE("Foe Abra's White Herb restored its status!");
    } THEN {
...
SINGLE_BATTLE_TEST("White Herb wont have time to activate if it is knocked off or stolen by Thief")
{
    ...
    GIVEN {
        ...
        if (move == MOVE_THIEF) {
            MESSAGE("Foe Alakazam stole Slugma's White Herb!");
        }
        ...
        if (move == MOVE_KNOCK_OFF) {
            MESSAGE("Foe Alakazam knocked off Slugma's White Herb!");
        }
        NONE_OF {
            ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, player);
            MESSAGE("Alakazam's White Herb restored its status!");
