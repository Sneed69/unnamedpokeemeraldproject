...
SINGLE_BATTLE_TEST("Toxic Chain inflicts bad poison when attacking")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_SKUNTANK) { Ability(ABILITY_TOXIC_CHAIN); }
        ...
        MESSAGE("Foe Alakazam was badly poisoned by Skuntank's Toxic Chain!");
...
SINGLE_BATTLE_TEST("Toxic Chain inflicts bad poison on any hit of a multi-hit move")
{
    GIVEN {
        ...
        PLAYER(SPECIES_SKUNTANK) { Ability(ABILITY_TOXIC_CHAIN); }
        OPPONENT(SPECIES_ALAKAZAM) { Item(ITEM_PECHA_BERRY); }
    } WHEN {
...
DOUBLE_BATTLE_TEST("Toxic Chain can inflict bad poison on both foes")
{
    GIVEN {
        ...
        MESSAGE("Foe Abra was badly poisoned by Skuntank's Toxic Chain!");
...
SINGLE_BATTLE_TEST("Toxic Chain makes Lum/Pecha Berry trigger before being knocked off")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_SKUNTANK) { Ability(ABILITY_TOXIC_CHAIN); }
        OPPONENT(SPECIES_ALAKAZAM) { Item(item); HP(600); }
    } WHEN {
        ...
        NONE_OF {
            ...
            MESSAGE("Okidogi knocked off Foe Alakazam's Pecha Berry!");
            MESSAGE("Okidogi knocked off Foe Alakazam's Lum Berry!");
        }
    } THEN {
