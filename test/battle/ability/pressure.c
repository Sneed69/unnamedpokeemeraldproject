...
{
    GIVEN {
        PLAYER(SPECIES_ALAKAZAM) { MovesWithPP({MOVE_POUND, 35}); }
        OPPONENT(SPECIES_ALAKAZAM) { Ability(ABILITY_PRESSURE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_ALAKAZAM) { MovesWithPP({MOVE_SWIFT, 20}); }
        PLAYER(SPECIES_ABRA);
        OPPONENT(SPECIES_ALAKAZAM) { Ability(ABILITY_PRESSURE); }
        OPPONENT(SPECIES_ABRA) { Ability(ABILITY_PRESSURE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_ALAKAZAM) { MovesWithPP({MOVE_IMPRISON, 10}, {MOVE_SNATCH, 10}); }
        OPPONENT(SPECIES_ALAKAZAM) { Ability(ABILITY_PRESSURE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_ALAKAZAM) { MovesWithPP({MOVE_SPIKES, 20}, {MOVE_STEALTH_ROCK, 20}, {MOVE_TOXIC_SPIKES, 20}); }
        OPPONENT(SPECIES_ALAKAZAM) { Ability(ABILITY_PRESSURE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_ALAKAZAM) { MovesWithPP({MOVE_STICKY_WEB, 20}); }
        OPPONENT(SPECIES_ALAKAZAM) { Ability(ABILITY_PRESSURE); }
    } WHEN {
