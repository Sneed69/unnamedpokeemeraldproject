...
SINGLE_BATTLE_TEST("Hyper Cutter prevents intimidate")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_CORPHISH) { Ability(ABILITY_HYPER_CUTTER); }
    } WHEN {
        ...
        MESSAGE("Foe Corphish's Hyper Cutter prevents Attack loss!");
...
SINGLE_BATTLE_TEST("Hyper Cutter doesn't prevent Attack reduction from burn")
{
    GIVEN {
        ...
        MESSAGE("Foe Corphish was burned!");
    } THEN {
...
SINGLE_BATTLE_TEST("Hyper Cutter is ignored by Mold Breaker")
{
    GIVEN {
        ...
        MESSAGE("Foe Corphish's Attack fell!");
        NONE_OF {
            ABILITY_POPUP(opponent, ABILITY_HYPER_CUTTER);
            MESSAGE("Foe Corphish's Hyper Cutter prevents Attack loss!");
        }
...
SINGLE_BATTLE_TEST("Hyper Cutter doesn't prevent Attack stage reduction from moves used by the user (Topple Psyche)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffectSelf(MOVE_TOPPLE_PSYCHE, MOVE_EFFECT_ATK_DEF_DOWN) == TRUE);
        PLAYER(SPECIES_ALAKAZAM);
        OPPONENT(SPECIES_CORPHISH) { Ability(ABILITY_HYPER_CUTTER); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_TOPPLE_PSYCHE); }
        TURN {}
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_TOPPLE_PSYCHE, opponent);
        MESSAGE("Foe Corphish's Attack fell!");
        MESSAGE("Foe Corphish's Defense fell!");
    } THEN {
        EXPECT_EQ(opponent->statStages[STAT_ATK], DEFAULT_STAT_STAGE - 1);
    }
}

SINGLE_BATTLE_TEST("Hyper Cutter doesn't prevent Attack stage reduction from moves used by the user (Superpower)")
{
    GIVEN {
        ...
        MESSAGE("Foe Corphish's Attack harshly fell!");
    } THEN {
...
SINGLE_BATTLE_TEST("Hyper Cutter doesn't prevent Topsy-Turvy")
{
    GIVEN {
        ...
        MESSAGE("Foe Corphish's Attack sharply rose!");
        ANIMATION(ANIM_TYPE_MOVE, MOVE_TOPSY_TURVY, player);
        MESSAGE("Foe Corphish's stat changes were all reversed!");
    } THEN {
        EXPECT_EQ(opponent->statStages[STAT_ATK], DEFAULT_STAT_STAGE - 2);
    }
}
...
SINGLE_BATTLE_TEST("Hyper Cutter doesn't prevent Spectral Thief from resetting positive Attack stage changes")
{
    GIVEN {
        ...
        MESSAGE("Alakazam stole the target's boosted stats!");
    } THEN {
        EXPECT_EQ(opponent->statStages[STAT_ATK], DEFAULT_STAT_STAGE);
    }
}
