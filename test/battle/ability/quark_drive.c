...
SINGLE_BATTLE_TEST("Quark Drive boosts either Attack or Special Attack, not both")
{
    ...
    PARAMETRIZE { species = SPECIES_GALLADE; move = MOVE_TACKLE; }
    PARAMETRIZE { species = SPECIES_GALLADE; move = MOVE_ROUND; }

    PARAMETRIZE { species = SPECIES_KADABRA; move = MOVE_TACKLE; }
    PARAMETRIZE { species = SPECIES_KADABRA; move = MOVE_ROUND; }
    ...
    GIVEN {
        ...
        if ((move == MOVE_TACKLE && species == SPECIES_GALLADE) || (move == MOVE_ROUND && species == SPECIES_KADABRA))
...
SINGLE_BATTLE_TEST("Quark Drive ability pop up activates only once during the duration of electric terrain")
{
    ...
    GIVEN {
        PLAYER(SPECIES_KADABRA) { Ability(ABILITY_QUARK_DRIVE); }
        OPPONENT(SPECIES_ALAKAZAM);
    } WHEN {
        TURN { MOVE(player, MOVE_ELECTRIC_TERRAIN); }
