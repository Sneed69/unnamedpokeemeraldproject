    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            ...
            for (k = 0; k < FRONTIER_PARTY_SIZE; k++)
            {
                ...
                if (personality & 1)
                    targetAbilities[0] = gSpeciesInfo[targetSpecies].abilities[1];
                else
                    targetAbilities[0] = gSpeciesInfo[targetSpecies].abilities[0];
                targetAbilities[1] = gSpeciesInfo[targetSpecies].innate;
