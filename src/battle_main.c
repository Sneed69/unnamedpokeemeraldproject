...
static u32 GeneratePartyHash(const struct Trainer *trainer, u32 i)
{
    const u8 *buffer  = (const u8 *) &trainer->party.TrainerMon[i];
    u32 n = sizeof(*trainer->party.TrainerMon);
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            partyData = trainer->party.TrainerMon;
...
u32 GetBattlerTotalSpeedStat(u8 battlerId)
{
    ...
    // other abilities
    if (ability == ABILITY_TOXIC_BOOST && gBattleMons[battlerId].status1 & STATUS1_PSN_ANY)
        speed = (speed * 150) / 100;
    if (ability == ABILITY_FLARE_BOOST && gBattleMons[battlerId].status1 & STATUS1_BURN)
        speed = (speed * 150) / 100;
        ...
        || gStatuses4[battlerAtk] & STATUS4_ELECTRIFIED)
    {
        gBattleStruct->dynamicMoveType = TYPE_ELECTRIC | F_DYNAMIC_TYPE_2;
             ...
             && ((attackerAbility == ABILITY_PIXILATE && (ateType = TYPE_FAIRY))
                 || (attackerAbility == ABILITY_COLD_EMBRACE && (ateType = TYPE_ICE))
