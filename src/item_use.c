...
void ItemUseOutOfBattle_Bike(u8 taskId)
{
    ...
    {
        if (Overworld_IsBikingAllowed() && IsBikingDisallowedByPlayer() == 0)
        {
...
static bool32 CanFish(void)
{
    ...
    else
    {
        if (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehavior) && MapGridGetCollisionAt(x, y) == 0)
            return TRUE;
        if (MetatileBehavior_IsBridgeOverWaterNoEdge(tileBehavior))
            return TRUE;
    }
...
{
    if (ItemfinderCheckForHiddenItems(gMapHeader.events, taskId))
...
{
    if (ObjectEventCheckHeldMovementStatus(&gObjectEvents[GetObjectEventIdByLocalIdAndMap(OBJ_EVENT_ID_PLAYER, 0, 0)]))
...
void ItemUseOutOfBattle_Proficiency(u8 taskId)
{
    gItemUseCB = ItemUseCB_Proficiency;
    SetUpItemUseCallback(taskId);
}
...
static void Task_UseRepel(u8 taskId)
{
    if (!IsSEPlaying())
    {
        VarSet(VAR_REPEL_STEP_COUNT, ItemId_GetHoldEffectParam(gSpecialVar_ItemId) * REPEL_STEP_MULTIPLIER);
