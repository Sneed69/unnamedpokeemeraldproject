...
static u8 GetPartyBoxPaletteFlags(u8 slot, u8 animNum)
{
    ...
    if (PartyBoxPal_ParnterOrDisqualifiedInArena(slot))
...
static u8 GetPartyMenuActionsType(struct Pokemon *mon)
{
    ...
    case PARTY_MENU_TYPE_FIELD:
        if (InMultiPartnerRoom() || GetMonData(mon, MON_DATA_IS_EGG))
    ...
    // PARTY_MENU_TYPE_EGG_MOVE_TUTOR
...
void Task_AbilityCapsule(u8 taskId)
{
    ...
    case 0:
        ...
        StringCopy(gStringVar3, gAbilitiesInfo[GetAbilityBySpecies(tSpecies, tAbilityNum ^ 1)].name);
...
static void ShowMoveSelectWindow(u8 slot)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (StringLength(GetMoveName(move)) < 13)
            AddTextPrinterParameterized(windowId, FONT_NORMAL, GetMoveName(move), 8, (i * 16) + 1, TEXT_SKIP_DRAW, NULL);
        else
            AddTextPrinterParameterized(windowId, FONT_NARROW, GetMoveName(move), 8, (i * 16) + 1, TEXT_SKIP_DRAW, NULL);
        if (move != MOVE_NONE)
            moveCount++;
    }
...
static bool8 TrySwitchInPokemon(void)
{
    ...
    if (IsMultiBattle() && (slot == 1 || slot == 4 || slot == 5))
    {
