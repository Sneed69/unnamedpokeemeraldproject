...
static void Task_PrintIVsEVsPage(u8);
...
static void PrintHeldItemName(void)
{
    ...
    x = GetStringCenterAlignXOffset(FONT_NORMAL, text, 90) + 6;
...
static void BufferStat(u8 *dst, s8 natureMod, u32 stat, u32 strId, u32 n)
{
    ...
    static const u8 sTextNatureNeutral[] = _("{COLOR 1}{SHADOW 0}");
...
static void PrintMoveNameAndPP(u8 moveIndex)
{
    ...
    if (move != 0)
    {
        ...
        if (StringLength(GetMoveName(move)) < 13)
...
static void PrintNewMoveDetailsOrCancelText(void)
{
    ...
    else
    {
        ...
        if (StringLength(GetMoveName(move)) < 13)
            PrintTextOnWindow(windowId1, GetMoveName(move), 0, 65, 0, color);
        else
            PrintNarrowTextOnWindow(windowId1, GetMoveName(move), 0, 65, 0, color);
...
static void SetMonTypeIcons(void)
{
    ...
    {
        SetTypeSpritePosAndPal(gSpeciesInfo[summary->species].types[0], 118, 40, SPRITE_ARR_ID_TYPE);
        if (gSpeciesInfo[summary->species].types[0] != gSpeciesInfo[summary->species].types[1])
        {
            SetTypeSpritePosAndPal(gSpeciesInfo[summary->species].types[1], 151, 40, SPRITE_ARR_ID_TYPE + 1);
