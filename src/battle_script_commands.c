        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if ((B_SPLIT_EXP < GEN_6 || gBattleMoveDamage == 0)) // only give exp share bonus in later gens if the mon wasn't sent out
                    {
                    ...
                    else if (!gBattleStruct->teamGotExpMsgPrinted) // Print 'the rest of your team got exp' message once, when all of the sent-in mons were given experience
                    {
...
static void Cmd_switchineffects(void)
{
    ...
    else
    {
        ...
        if ((battlerAbility == ABILITY_INTIMIDATE || battlerAbility == ABILITY_SUPERSWEET_SYRUP || battlerAbility == ABILITY_DOWNLOAD || battlerAbility == ABILITY_UNTHREATENING)
    ...
    if (statValue <= -1) // Stat decrease.
    {
                 ...
                 && notProtectAffected != TRUE && JumpIfMoveAffectedByProtect(gCurrentMove))
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
              ...
              || CanAbilityPreventStatLoss(battlerAbility, (GetBattlerAbility(gBattlerAttacker) == ABILITY_INTIMIDATE) || GetBattlerAbility(gBattlerAttacker) == ABILITY_UNTHREATENING))
