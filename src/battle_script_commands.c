...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        ...
        else if ((gBattleWeather & B_WEATHER_HAIL) && (move == MOVE_BLIZZARD || gBattleMoves[move].effect == EFFECT_HURRICANE))
        {
            // thunder/hurricane ignore acc checks in rain unless target is holding utility umbrella
            JumpIfMoveFailed(7, move);
            return TRUE;
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        case ITEM_HEAVY_BALL:
            ballMultiplier = GetPokedexHeightWeight(SpeciesToNationalPokedexNum(gBattleMons[gBattlerTarget].species), 1) / 10;
            if (ballMultiplier == 0)
                ballMultiplier = 1;
