...
s32 CalcCritChanceStage(u8 battlerAtk, u8 battlerDef, u32 move, bool32 recordAbility)
{
    ...
    else if (abilityDef == ABILITY_BATTLE_ARMOR || abilityDef == ABILITY_SHELL_ARMOR || (gBattleTypeFlags & BATTLE_TYPE_LEGENDARY && GET_BATTLER_SIDE2(gBattlerTarget) == B_SIDE_OPPONENT))
    {
        if (recordAbility)
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_RECOIL_33_STATUS: // Flare Blitz - can burn, Volt Tackle - can paralyze
                ...
                break;
            case MOVE_EFFECT_RECOIL_33_SPEED_UP:
                gBattleScripting.savedDmg = gHpDealt / 3;
                if (gBattleScripting.savedDmg == 0)
                    gBattleScripting.savedDmg = 1;

                BattleScriptPush(gBattlescriptCurrInstr + 1);
                gBattlescriptCurrInstr = BattleScript_MoveEffectRecoilSpeedUp;
                break;
...
static void Cmd_getexp(void)
{
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (!GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_IS_EGG))
                {
                    ...
                    //give xp to fainted participants but dont give shared xp to fainted mons
                    if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP) || gBattleMoveDamage > 0)
                    {
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (gBattleMoveDamage > 0 && GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) != MAX_LEVEL)
            {
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        case ITEM_HEAVY_BALL:
            ballMultiplier = GetPokedexHeightWeight(SpeciesToNationalPokedexNum(gBattleMons[gBattlerTarget].species), 1) / 100;
            if (ballMultiplier == 0)
                ballMultiplier = 1;
            else if (ballMultiplier > 30)
                ballMultiplier = 30;
            break;
