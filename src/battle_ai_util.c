    ...
    if (gMovesInfo[move].power && !isDamageMoveUnusable)
    {
        ...
        if (!gBattleStruct->zmove.active)
        {
            ...
            case EFFECT_LEVEL_DAMAGE:
                ...
                break;
            case EFFECT_PSYWAVE:
                simDamage.expected = 50 * (aiData->abilities[battlerAtk] == ABILITY_PARENTAL_BOND ? 2 : 1);
...
bool32 AI_IsBattlerGrounded(u32 battlerId)
{
    ...
    if (holdEffect == HOLD_EFFECT_IRON_BALL)
        ...
        return FALSE;
    else if (AI_DATA->abilities[battlerId] == ABILITY_FLIGHTLESS)
        return TRUE;
...
bool32 ShouldSetSun(u32 battlerAtk, u32 atkAbility, u32 holdEffect)
{
    ...
    if (holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA
     && (atkAbility == ABILITY_CHLOROPHYLL
      || atkAbility == ABILITY_DESERT_DWELLER
