...
static u8 ChooseWildMonIndex_Water(void)
{
    ...
    if (rand < ENCOUNTER_CHANCE_WATER_MONS_SLOT_0)
        ...
        wildMonIndex = 4;
    else if (rand < ENCOUNTER_CHANCE_WATER_MONS_SLOT_5)
        wildMonIndex = 5;
    else if (rand < ENCOUNTER_CHANCE_WATER_MONS_SLOT_6)
        wildMonIndex = 6;
    else if (rand < ENCOUNTER_CHANCE_WATER_MONS_SLOT_7)
        wildMonIndex = 7;
    else if (rand < ENCOUNTER_CHANCE_WATER_MONS_SLOT_8)
        wildMonIndex = 8;
    else if (rand < ENCOUNTER_CHANCE_WATER_MONS_SLOT_9)
        wildMonIndex = 9;
    else if (rand < ENCOUNTER_CHANCE_WATER_MONS_SLOT_10)
        wildMonIndex = 10;
    else
        wildMonIndex = 11;
...
static u8 ChooseWildMonIndex_Rock(void)
{
    ...
    if (rand < ENCOUNTER_CHANCE_ROCK_SMASH_MONS_SLOT_0)
        ...
        wildMonIndex = 8;
    else if (rand < ENCOUNTER_CHANCE_ROCK_SMASH_MONS_SLOT_9)
        wildMonIndex = 9;
    else if (rand < ENCOUNTER_CHANCE_ROCK_SMASH_MONS_SLOT_10)
        wildMonIndex = 10;
    else
        wildMonIndex = 11;
...
static u8 ChooseWildMonIndex_Cut(void)
{
    ...
    if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_0)
        wildMonIndex = 0;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_1)
        wildMonIndex = 1;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_2)
        wildMonIndex = 2;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_3)
        wildMonIndex = 3;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_4)
        wildMonIndex = 4;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_5)
        wildMonIndex = 5;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_6)
        wildMonIndex = 6;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_7)
        wildMonIndex = 7;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_8)
        wildMonIndex = 8;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_9)
        wildMonIndex = 9;
    else if (rand < ENCOUNTER_CHANCE_CUT_MONS_SLOT_10)
        wildMonIndex = 10;
    else
        wildMonIndex = 11;
...
static u8 ChooseWildMonIndex_Fishing(u8 rod)
{
    ...
    case GOOD_ROD:
        if (rand < ENCOUNTER_CHANCE_FISHING_MONS_GOOD_ROD_SLOT_2)
            wildMonIndex = 2;
        else if (rand < ENCOUNTER_CHANCE_FISHING_MONS_GOOD_ROD_SLOT_3)
            wildMonIndex = 3;
        else
            wildMonIndex = 4;
...
static bool8 WildEncounterCheck(u32 encounterRate, bool8 ignoreAbility)
{
    ...
    if (!ignoreAbility && !GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG))
    {
        ...
        if (ability == ABILITY_STENCH && gMapHeader.mapLayoutId == LAYOUT_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR)
            ...
            encounterRate *= 2;
        else if ((ability == ABILITY_SAND_VEIL || ability == ABILITY_CRYSTAL_SHROUD) && gSaveBlock1Ptr->weather == WEATHER_SANDSTORM)
            encounterRate /= 2;
        else if ((ability == ABILITY_SNOW_CLOAK || ability == ABILITY_CRYSTAL_SHROUD) && gSaveBlock1Ptr->weather == WEATHER_SNOW)
            encounterRate /= 2;
        ...
        if (MetatileBehavior_IsLandWildEncounter(curMetatileBehavior))
        {
            ...
            else
            {
                ...
                return FALSE;
                 ...
                 || (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING) && MetatileBehavior_IsBridgeOverWater(curMetatileBehavior)))
        {
...
void CutWildEncounter(void)
{
    ...
    if (headerId != 0xFFFF)
    {
        ...
        if (wildPokemonInfo == NULL)
        {
         ...
         && TryGenerateWildMon(wildPokemonInfo, WILD_AREA_TREES, WILD_CHECK_REPEL | WILD_CHECK_KEEN_EYE))
        {
            BattleSetup_StartWildBattle();
            gSpecialVar_Result = TRUE;
        }
