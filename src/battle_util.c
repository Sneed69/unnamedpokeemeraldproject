...
bool32 IsMoveMakingContact(u16 move, u8 battlerAtk)
{
    ...
    if (!(gBattleMoves[move].flags & FLAG_MAKES_CONTACT))
    {
        if (gBattleMoves[move].effect == EFFECT_SHELL_SIDE_ARM && gBattleStruct->swapDamageCategory)
           ...
           || GetBattlerAbility(battlerAtk) == ABILITY_GHOST_TOUCH)
    {
        return FALSE;
    }
    else
...
static void MulByTypeEffectiveness(u16 *modifier, u16 move, u8 moveType, u8 battlerDef, u8 defType, u8 battlerAtk, bool32 recordAbilities)
{
    ...
    if (gBattleMoves[move].effect == EFFECT_FREEZE_DRY && defType == TYPE_WATER)
        mod = UQ_4_12(2.0);
    if (moveType == TYPE_GROUND && defType == TYPE_FLYING && (IsBattlerGrounded(battlerDef) || gBattleMoves[move].flags & FLAG_IGNORE_GROUND_IMMUNITY) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
...
static u16 CalcTypeEffectivenessMultiplierInternal(u16 move, u8 moveType, u8 battlerAtk, u8 battlerDef, bool32 recordAbilities, u16 modifier)
{
    ...
    if (moveType == TYPE_GROUND && !IsBattlerGrounded2(battlerDef, TRUE) && !(gBattleMoves[move].flags & (FLAG_DMG_UNGROUNDED_IGNORE_TYPE_IF_FLYING | FLAG_IGNORE_GROUND_IMMUNITY)))
    {
